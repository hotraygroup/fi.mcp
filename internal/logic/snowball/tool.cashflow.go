...
// CashFlowArgs 定义 cashflow 工具的输入参数
...
func NewCashFlowTool(_mcp types.MCPProvider) (*mcp.Tool, func(context.Context, *mcp.CallToolRequest, CashFlowArgs) (*mcp.CallToolResult, any, error)) {
	tool := &mcp.Tool{
		Name:        "cashflow",
		Description: "获取公司现金流量表",
	}

	handler := func(ctx context.Context, req *mcp.CallToolRequest, args CashFlowArgs) (*mcp.CallToolResult, any, error) {
		if args.Count == "" {
			args.Count = "6"
		}
		...
		url := fmt.Sprintf(_mcp.GetServiceContext().Config.DataSource.Snowball.CashFlowURL, args.Symbol, args.Count, time.Now().UnixMilli())
