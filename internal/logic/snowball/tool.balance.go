...
// BalanceArgs 定义 balance 工具的输入参数
...
func NewBalanceTool(_mcp types.MCPProvider) (*mcp.Tool, func(context.Context, *mcp.CallToolRequest, BalanceArgs) (*mcp.CallToolResult, any, error)) {
	tool := &mcp.Tool{
		Name:        "balance",
		Description: "获取公司资产负债表",
	}

	handler := func(ctx context.Context, req *mcp.CallToolRequest, args BalanceArgs) (*mcp.CallToolResult, any, error) {
		if args.Count == "" {
			args.Count = "6"
		}
		...
		url := fmt.Sprintf(_mcp.GetServiceContext().Config.DataSource.Snowball.BalanceURL, args.Symbol, args.Count, time.Now().UnixMilli())
